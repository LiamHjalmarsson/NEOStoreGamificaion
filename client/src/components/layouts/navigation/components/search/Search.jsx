...
const Search = ({ open, setSearch, searchValue }) => {
    ...
    return (
        <div className={`absolute inset-0 z-20 transition-opacity ${open ? "opacity-100" : "opacity-0 pointer-events-none"} transition duration-500`}>
            <div className={`${open ? "translate-y-0" : "-translate-y-full"} max-w-xl w-full mx-auto fixed top-44 left-0 right-0 transition-transform duration-500 `}>
                <label htmlFor="search" className="">
                    <div className="relative flex justify-center items-center z-30">
                        ...
                        <input
                            type="search"
                            id="search"
                            className="block w-full p-6 pl-12 pr-36 bg-stone-200 dark:bg-stone-800 shadow-md rounded-md outline-none transition duration-500 shadow-stone-800"
            ...
            <div className={`${searchValue.length > 0 ? "visible" : "invisible"} max-w-xl flex flex-col gap-4 w-full mx-auto fixed top-72 left-0 right-0 transition duration-500 bg-stone-200 dark:bg-stone-800 p-4 rounded-md`}>
